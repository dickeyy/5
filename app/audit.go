...
type AuditEntryResponse struct {
	...
	Source              structs.AuditSource `json:"source"`
	Action              string              `json:"action"`
	ResourceType        string              `json:"resource_type"`
	ResourceID          string              `json:"resource_id"`
	Result              structs.AuditResult `json:"result"`
...
func (s *AuditService) List(ctx context.Context, guildContext *GuildStaffContext, input AuditListInput) (*AuditListResponse, error) {
	...
	resultValue := structs.AuditResult(strings.TrimSpace(input.Result))
	if resultValue != "" && !validAuditResult(resultValue) {
		return nil, validationAuditError("result is invalid")
	}

	result, err := s.store.ListAuditLogEntriesFiltered(ctx, storage.ListAuditLogEntriesParams{
...
func validAuditResult(result structs.AuditResult) bool {
	switch result {
	case structs.AuditResultSuccess, structs.AuditResultFailure:
		return true
	default:
		return false
	}

...
func TestOptionalModuleLogicalMigrationsJoinCentralLedger(t *testing.T) {
	...
	if len(ledger) != 8 || ledger[5].Name != "optional_module_registry_0100" || ledger[6].Name != "ticket_lifecycle_0110" || ledger[7].Name != "honeypot_triggers_0300" {
		t.Fatalf("unexpected reconciled ledger: %+v", ledger)
	}
	if strings.Contains(migration0006Source, "internal/modules") || strings.Contains(migration0007Source, "internal/modules") || strings.Contains(migration0008Source, "internal/modules") {
